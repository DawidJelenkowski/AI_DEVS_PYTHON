...
query = ("""I have been working on a desktop app project for macOS for a few months now. 
         ...
         I have several others, which necessitates careful time management and focus. 
         I am faced with the decision of choosing between two paths: 
...
conversation = [
    SystemMessage(content="""Act an expert in mental models, critical thinking, and making complex, strategic decisions. 
                  Use markdown syntax to format your responses throughout the conversation."""),
    HumanMessage(content=f"""{query}. Can you brainstorm three different possible strategies 
                 that I could take to effectively create new content and 
                 do this consistently while maintaining my energy, life balance, and overall quality of the content I produce? 
...
chat_and_log("""For each solution, evaluate their potential, pros and cons, effort needed, 
             difficulty, challenges and expected outcomes. Assign success rate and confidence level for each option.""")
chat_and_log("""Extend each solution by deepening the thought process. 
             Generate different scenarios, strategies of implementation that include external resources 
             and how to overcome potential unexpected obstacles.""")
chat_and_log("""For each scenario, generate a list of tasks 
             that need to be done to implement the solution.""")
chat_and_log("""Based on the evaluations and scenarios, rank the solutions in order. 
             Justify each ranking and offer a final solution.""")

conversation_text = "\n\n".join(
    [f"## {type(message).__name__}:\n\n{message.content}" for message in conversation])
with open('/home/xbloc/respos/2nd-devs-python/lanchain_python/17_tree/result.md', 'w') as f:
